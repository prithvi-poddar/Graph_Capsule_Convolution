...
class Context_Encoder_V2(nn.Module):
    def __init__(self, 
        ...
        self.caps2 = Secondary_Capsule(input_dim=hidden_dims[1]*p, 
                                       ...
                                       device=device)
        self.batch_norm = nn.BatchNorm1d(hidden_dims[2]*p, affine=False, device=device)
        self.context = nn.Linear(in_features=hidden_dims[2]*p*2+1,
                                 out_features=out_feats)
        self.activation = nn.ReLU()
        self.device = device
        self.to(device)
    ...
    def forward(self, agent_idx, time, data=None, X=None, L=None):
        ...
        agent_x = torch.unsqueeze(x_caps[agent_idx,:], 0)
        peers_x = peers = torch.cat((x_caps[:agent_idx,:],x_caps[agent_idx+1:,:]), dim=0)
        peers_x = torch.sum(peers_x, dim=0, keepdim=True)

        x = self.activation(self.context(torch.cat((time, agent_x, peers_x), dim=1)))
        return x